...
public class SpotSurfaceFire extends Signal {
	...
	public static final double flameLengthMin = 0.0;
	public static final double flameLengthMax = 30.5;
	...
	protected double m_flameLength;
	...
	public SpotSurfaceFire(SpotAlgorithm.SpotSource source, double ridgetopToValleyDistance, double ridgetopToValleyElevation, double coverHeight, double windSpeedAt20Ft, double flameLength) {
		...
		assert m_flameLength >= flameLengthMin && m_flameLength <= flameLengthMax;
	...
	public SpotSurfaceFire(SpotSurfaceFire right) {
	    init();
	...
	public void setFlameLength(double flameLength) {
		assert flameLength >= flameLengthMin && flameLength <= flameLengthMax;
	...
	protected void update() {
		...
		m_firebrandHeight = SpotAlgorithm.firebrandHeightFromSurfaceFire(m_flameLength, m_windSpeedAt20Ft);
		m_criticalCover = SpotAlgorithm.criticalCoverHeight(m_firebrandHeight, m_coverHeight);
		m_flatSpottingDistance = SpotAlgorithm.flatTerrainSpotDistanceFromSurfaceFire(m_firebrandHeight, m_windSpeedAt20Ft, m_coverHeight);
		m_mtnSpottingDistance = SpotAlgorithm.mountainTerrainSpotDistance(m_flatSpottingDistance, m_source, m_distance, m_elevation);
	}
	
	public static boolean equal(SpotSurfaceFire lhs, SpotSurfaceFire rhs) {
		return( abs( lhs.coverHeight() - rhs.coverHeight() ) < Smidgen
			       ...
			       && abs( lhs.flameLength() - rhs.flameLength() ) < Smidgen
			       && lhs.source() == rhs.source()
			       && abs( lhs.windSpeedAt20Ft() - rhs.windSpeedAt20Ft() ) < Smidgen );
	}
	
	public static boolean notEqual(SpotSurfaceFire lhs, SpotSurfaceFire rhs) {
